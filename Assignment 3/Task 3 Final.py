...
X = sm.add_constant(X)
motor_updrs_model = sm.OLS(y_motor_updrs,X).fit()
motor_updrs_pred = motor_updrs_model.predict(X)
motor_updrs_model_details = motor_updrs_model.summary()
X = sm.add_constant(X)
total_updrs_model = sm.OLS(y_total_updrs,X).fit()
total_updrs_pred = total_updrs_model.predict(X)
total_updrs_model_details = total_updrs_model.summary()
print(motor_updrs_model_details)
print("-"*100)
print(total_updrs_model_details)
...
motor_updrs_model.fit(X_train_std, y_motor_train)
total_updrs_model.fit(X_train_std, y_total_train)

# Making predictions on the test set
motor_updrs_predictions = motor_updrs_model.predict(X_test_std)
total_updrs_predictions = total_updrs_model.predict(X_test_std)
# Mean Absolute Error
